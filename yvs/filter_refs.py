...
def choose_best_version(user_prefs, bible, query):
    ...
    if not chosen_version and "version" in user_prefs:
        chosen_version = core.get_version(bible["versions"], user_prefs["version"])
...
def get_result(book, query, chosen_version, user_prefs):
    ...
    result["title"] = "{book} {chapter}".format(book=book["name"], chapter=chapter)
...
def get_result_list(query_str):
    ...
    return [
        get_result(book, query, chosen_version, user_prefs)
