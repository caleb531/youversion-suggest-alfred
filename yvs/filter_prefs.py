...
def get_pref_defs(user_prefs):
    return [
        ...
        },
        {
            ...
            "short_name": "version",
            "values": [
                get_version_value(version)
                for version in sorted(
                    core.get_versions(user_prefs["language"]), key=itemgetter("name")
                )
            ],
...
def get_ref_format_values(user_prefs):
    ...
    return [get_ref_format_value(ref_format, ref) for ref_format in ref_formats]
...
def get_value_result_list(user_prefs, pref_def, query_str):
    results = [
        ...
        if not query_str or if_query_str_matches(value["name"], query_str)
    ]
    ...
    if pref_match:
        ...
        for pref_def in pref_defs:
            ...
            if core.normalize_query_str(pref_def["id"]) == pref_key_query:
                ...
                results = get_value_result_list(user_prefs, pref_def, pref_value_query)
                break
