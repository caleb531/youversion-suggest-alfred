...
class ReferenceParser(web.YVParser):
    ...
    break_elems = {"li1", "q", "q1", "q2", "qc", "qr", "qm1", "qm2"}

    # Associates the given reference object with this parser instance
    def __init__(self, ref, include_verse_numbers=False, include_line_breaks=False):
        super().__init__()
        self.ref = ref
    ...
    def is_in_verse(self):
        ...
        return any(
            self.in_verse
            and (verse_num >= verse_start and (not verse_end or verse_num <= verse_end))
    ...
    def is_in_verse_content(self):
        return self.is_in_verse() and self.in_verse_content and not self.in_verse_note
    ...
    def is_in_verse_label(self):
        return (
            self.is_in_verse()
            and self.include_verse_numbers
            and self.in_verse_label
    ...
    def get_verse_nums_from_verse_attrs(self, verse_attrs):
        if verse_attrs.get("data-usfm"):
            verse_num_strs = re.findall(
                r"(\w{3})\.(\d+)\.(\d+)", verse_attrs["data-usfm"]
            )
