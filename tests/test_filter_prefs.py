...
def test_show_languages():
    ...
    case.assertEqual(len(results), len(glob.glob("yvs/data/bible/bible-*.json")))
...
def test_filter_languages():
    ...
    case.assertEqual(results[0]["title"], "Español (América Latina)")
...
def test_filter_versions():
    ...
    case.assertEqual(results[0]["title"], "New International Reader’s Version (NIRV)")
    case.assertEqual(results[0].get("valid", True), True)
    case.assertEqual(
        results[0]["variables"],
        {
...
def test_filter_preferences_name_partial():
    ...
    case.assertEqual(results[0]["title"], "English Standard Version 2016 (ESV)")
